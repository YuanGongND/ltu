...
# @File    : combine_all_v2.py

# small updates after the original submission, i.e., will be slightly different than the original version
...
combine_json_list(['../data/openaqa_5.6M.json', # audio all, audio open and close 5.6m 5681708
                   ...
                   '../data/open_ended/joint_as_audio_speech_after_sub.json'], # add more joint questions with new prompts after intitial submission 682490
                   '../data/openasqa_9.6M_v2.json') # mix speech open/close and original

# check leading " " space in text entry
datafile_list = ['../data/openasqa_9.6M_v2.json']
for datafile in datafile_list:
    ...
    for i in range(len(full_mix)):
        if 'yuangong/voxceleb' in full_mix[i]['audio_id']:
            #print('fixed a problem')
            full_mix[i]['audio_id'] = full_mix[i]['audio_id'].replace('yuangong/voxceleb', 'yuangong/dataset/voxceleb')

print(len(full_mix), empty_cnt/len(full_mix), no_text_cnt/len(full_mix))
with open('../data/openasqa_10.3M_v2.json', 'w') as outfile:
    json.dump(full_mix, outfile, indent=1)

# terminal print:
# ../data/openasqa_10.3M_v2.json
# 10324230
# 10324230 0.5548384722153613 0.09659354741225254