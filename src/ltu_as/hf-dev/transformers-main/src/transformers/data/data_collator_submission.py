...
class DataCollatorForSeq2Seq:
    ...
    def gen_filename(self, filename):
        ...
        if 'mosei' in filename:
            filename = '/data/sls/scratch/yuangong/audiollm/src/data/prep_data_ltue/whisper_feat/mosei/whisper_large-v1/' + filename.split('/')[-1][:-len(ext) - 1] + '.npz'
        elif 'IEMOCAP' in filename:
            filename = '/data/sls/scratch/yuangong/audiollm/src/data/prep_data_ltue/whisper_feat/iemocap/whisper_large-v1/' + filename.split('/')[-1][:-len(ext) - 1] + '.npz'
        elif 'libritts' in filename:
            filename = '/data/sls/scratch/yuangong/audiollm/src/data/prep_data_ltue/whisper_feat/libritts/whisper_large-v1/' + filename.split('/')[-1][:-len(ext) - 1] + '.npz'
        elif 'FMA/audio_16k' in filename:
            filename = '/data/sls/scratch/yuangong/audiollm/src/data/prep_data_ltue/whisper_feat/fma_wav/whisper_large-v1/' + filename.split('/')[-1][:-len(ext) - 1] + '.npz'
        elif 'voxceleb' in filename:
            filename = '/data/sls/scratch/yuangong/audiollm/src/data/prep_data_ltue/whisper_feat/voxceleb/whisper_large-v1/' + filename.split('/')[-1][:-len(ext) - 1] + '.npz'
        else:
            filename = '/data/sls/scratch/yuangong/audiollm/src/data/prep_data_ltue/whisper_feat/whisper_large-v1/' + filename.split('/')[-1][:-len(ext) - 1] + '.npz'
        return filename

    def load_audio(self, filename):
        filename = self.gen_filename(filename)
        audio_feat = torch.tensor(np.load(filename)['arr_0'])
        return audio_feat
    ...
    def __call__(self, features, return_tensors=None):
        ...
        audio_att_mask = torch.ones([ori_att_mask.shape[0], 25]) # batch size [2, 25]
