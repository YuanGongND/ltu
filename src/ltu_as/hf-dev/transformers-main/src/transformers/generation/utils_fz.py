...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        """
        print('now in custom generation mode')
        ...
        print('before attention mask', model_kwargs)
        if model_kwargs.get("attention_mask", None) is None and requires_attention_mask and accepts_attention_mask:
            print('go into initialization loop')
        ...
        if is_greedy_gen_mode:
            ...
            print('now in sample generation mode')
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        print('ori att mask', ori_att_mask.shape)
        audio_att_mask = torch.ones((ori_att_mask.shape[0], audio_input.shape[1]), dtype=torch.bool, device=ori_att_mask.device)
        print('audio att mask', audio_att_mask.shape)
        model_kwargs['attention_mask'] = torch.concat([ori_att_mask, audio_att_mask], dim=1)
        print('concat att mask', model_kwargs['attention_mask'].shape)
        ...
        while True:
            ...
            if loop_idx != 0:
                audio_input = None
            # prepare model inputs, the attention_map and pos_id will extend for 1 more (generated) in the prepare_inputs_for_generation function
            #print('in sample, before process', model_kwargs)
            model_inputs = self.prepare_inputs_for_generation(input_ids, audio_input, **model_kwargs)
            #print(model_inputs)
            print(loop_idx, model_inputs['input_ids'].shape, model_inputs['attention_mask'].shape, model_inputs['position_ids'].shape, model_inputs['position_ids'])
            loop_idx += 1
            # print(model_inputs)
            # forward pass to get next token
            outputs = self(
