...
class ATModel(nn.Module):
    def __init__(self, n_layer=32, rep_dim=1280, mode='tl_down_tr_512_1_8'):
        ...
        self.time_tr = ResidualAttentionBlock(512, self.num_tatt_head)
        self.layer_tr = ResidualAttentionBlock(512, self.num_latt_head)
    ...
    def forward(self, audio_rep, time_resolution=10):
        ...
        audio_rep = audio_rep.reshape([B, num_layer, audio_len, 512]) # [B, 32, 25, 1280]
        audio_rep = audio_rep.permute([0, 2, 1, 3]) # [B, 25, 32, 1280]
        audio_rep = audio_rep.reshape([B * audio_len, num_layer, 512]) # [B*25, 32, 1280]
        audio_rep = self.layer_tr(audio_rep)  # [B*25, 32, 1280]
        audio_rep = torch.mean(audio_rep, dim=1)  # [B*25, 1280]
        audio_rep = audio_rep.reshape([B, audio_len, 512])
        return audio_rep
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.audio_proj = nn.Sequential(nn.LayerNorm(512, elementwise_affine=False), nn.Linear(512, config.hidden_size))
    ...
    def forward(
        ...
        if audio_input != None: # [B, 32, 25, 1280], 32 = layer, 25 = seq_len
            ...
            # hard norm to 50
            #audio_input = audio_input / 100
            #print(torch.max(audio_input))
        ...
        if audio_input != None:
            # np.save('/data/sls/scratch/yuangong/audiollm/sample_audio/audio_embed.npy', audio_input.float().detach().cpu().numpy())
            # np.save('/data/sls/scratch/yuangong/audiollm/sample_audio/text_embed.npy', inputs_embeds.float().detach().cpu().numpy())
            # exit()

            # audio_low = 1.
            # audio_input = audio_input / audio_low
            # #print('rescale audio embedding by factor ', audio_low)

            #print(torch.mean(torch.abs(audio_input)), torch.mean(torch.abs(inputs_embeds)))
        ...
        # # audio should always beening unmasked, shouldn't be concate everytime
        # if audio_input != None:
        #     audio_attention_mask = torch.ones((batch_size, audio_length), dtype=torch.bool, device=inputs_embeds.device)
        #     attention_mask = torch.concat([audio_attention_mask, attention_mask], dim=1)
        ...
        #print('last hidden', hidden_states.shape)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #print('now use custom audio llama')
    ...
    def prepare_inputs_for_generation(
        ...
        if attention_mask is not None and position_ids is None:
            #print('manually set pos_id and att_mask')
