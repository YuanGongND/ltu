...
class AudioLlamaPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, AudioLlamaModel):
...
class AudioLlamaModel(AudioLlamaPreTrainedModel):
    """
    ...
    def forward(
        ...
        if audio_input != None: # [B, t, f]
            audio_input = self.audio_proj(audio_input)  # [B, 512, 768]
            #audio_input = torch.nn.functional.avg_pool2d(audio_input, (16, 1)) # [B, 32, 768]
            assert audio_input.shape[1] == 8 * 64
            audio_input = audio_input.reshape(audio_input.shape[0], 8, 64, audio_input.shape[-1])
            audio_input = torch.mean(audio_input, dim=1)  # mean pool over the frequency dimension
...
class AudioLlamaForCausalLM(AudioLlamaPreTrainedModel):
...
class AudioLlamaForSequenceClassification(AudioLlamaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"lm_head.weight"]
