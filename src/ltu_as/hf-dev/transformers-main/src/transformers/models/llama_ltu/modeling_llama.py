...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        # TODO: newly added
        #self.audio_proj = nn.Linear(config.audio_hidden_size, config.hidden_size)
    ...
    def forward(
        ...
        if audio_input != None: # [B, t, f]
            ...
            # todo, need to change this in future inference, use hard norm instead
            # audio_input = torch.nn.functional.normalize(audio_input, dim=-1)
        ...
        if audio_input != None:
            # np.save('/data/sls/scratch/yuangong/audiollm/sample_audio/audio_embed.npy', audio_input.detach().cpu().numpy())
            # np.save('/data/sls/scratch/yuangong/audiollm/sample_audio/text_embed.npy', inputs_embeds.detach().cpu().numpy())
