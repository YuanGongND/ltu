    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if not isinstance(self.model, PreTrainedModel):
            if isinstance(unwrap_model(self.model), PreTrainedModel):
                ...
                selected_params = {}
                # Iterate over the loaded state dictionary and filter out the parameters based on name
                # TODO: this only saves audio releated params to save storage
                for name, param in state_dict.items():
                    if 'lora' in name or 'audio' in name:
                        selected_params[name] = param
                torch.save(selected_params, os.path.join(output_dir, WEIGHTS_NAME))
                del selected_params
        else:
...
def create_fz_optimizer(self):
    ...
    print('now in customer optimizer')
    ...
    if self.optimizer is None:
        ...
        print([n for n, p in opt_model.named_parameters()])

        optimizer_grouped_parameters = [
            {"params": [p for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad and 'audio' in n)], "weight_decay": self.args.weight_decay,},
            {"params": [p for n, p in opt_model.named_parameters() if (n not in decay_parameters and p.requires_grad and 'audio' in n)], "weight_decay": 0, },
            {"params": [p for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad and 'audio' not in n)], "weight_decay": 0, 'lr': 0},
        ]

        print('Total audio parameter number is : {:.3f} million'.format(sum(p.numel() for p in [p for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad and 'audio' in n)]) / 1e6))
        print('Total audio parameter number is : {:.3f} million'.format(sum(p.numel() for p in [p for n, p in opt_model.named_parameters() if (n not in decay_parameters and p.requires_grad and 'audio' in n)]) / 1e6))
        print('Total llm parameter number is : {:.3f} million'.format(sum(p.numel() for p in [p for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad and 'audio' not in n)]) / 1e6))
